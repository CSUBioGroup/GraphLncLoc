...
class lncRNALocalizer:
    def __init__(self, weightPath, in_dim=128, hidden_dim=64, n_classes=4, double_c4=False, map_location=None,
                 device=torch.device("cpu")):
        stateDict = torch.load(weightPath, map_location=map_location)
        if n_classes == 4 and double_c4:
            self.node_features = np.load(r'data/node_features_dc.npy', allow_pickle=True)
        if n_classes == 5:
            self.node_features = np.load(r'data/node_features_5.npy', allow_pickle=True)
        self.id2lab = ['Cytoplasm', 'Nucleus', 'Exosome', 'Ribosome', 'Cytosol']
        self.kmers2id = np.load(r'data/kmers2id.npy', allow_pickle=True).item()
    ...
    def predict(self, RNA):
        ...
        with g.local_scope():
            ...
            return {k: v for k, v in zip(self.id2lab[:self.n_classes], F.softmax(res, dim=1).data.numpy()[0])}
